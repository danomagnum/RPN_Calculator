...
class Application(tk.Frame):
  ...
  def save(self):
    ...
    filename = filedialog.asksaveasfilename(filetypes=[('RPN files', '.rpn'), ('all files', '*')], title='Load Session')
    if filename:
  ...
  def load(self):
    global interp
    filename = filedialog.askopenfilename(filetypes=[('RPN files', '.rpn'), ('all files', '*')], title='Load Session')
    if filename:
      f = open(filename, 'rb')
      tmp = pickle.load(f)
      f.close()
